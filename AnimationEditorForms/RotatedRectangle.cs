...
namespace AnimationEditorForms
{
    public class RotatedRectangle
    {
        ...
        {
            Vector2 newCircleCenter = RotatePoint(
                circle.Center, Origin + new Vector2(CollisionRectangle.X, CollisionRectangle.Y), Rotation);

            if (Math.Abs(newCircleCenter.X - CollisionRectangle.X - Origin.X)
                < circle.Radius + CollisionRectangle.Width / 2 &&
                Math.Abs(newCircleCenter.Y - CollisionRectangle.Y - Origin.Y)
                < circle.Radius + CollisionRectangle.Height / 2)
            {
                return true;
            }
