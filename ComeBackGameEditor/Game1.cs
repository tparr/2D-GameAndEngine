...
namespace ComeBackGameEditor
{
    public class Game1 : Game
    {
                ...
                if (_mousepositions.LeftButton == ButtonState.Pressed)
                {
                    if (_oldmouse.LeftButton == ButtonState.Released)
                    {
                        ...
                        if (_state == MouseStates.Select)
                        {
                            ...
                            {
                                if (MouseinTile(_mousepositions, door.TestBox))
                                {
                                    _form.DisplayDoor(door.TestBox.Width, door.TestBox.Height);
                                }
                            }
                        }
                                    ...
                                    default:
                                        ...
                                        else if (_state == MouseStates.Delete)
                                        {
                                            ...
                                            for (int i = 0; i < Doors.Count; i++)
                                                if (MouseinTile(_mousepositions, new Rectangle(Doors[i].TestBox.X, Doors[i].TestBox.Y, 32, 32)))
                                                    Doors.RemoveAt(i);
        ...
        protected override void Draw(GameTime gameTime)
        {
            ...
            foreach (Door door in Doors)
                _spriteBatch.Draw(_door, new Vector2(door.TestBox.X - _camerax, door.TestBox.Y - _cameray), Color.White);
